...
class ModelNSP_flan(nn.Module):
    def __init__(self, pretrained_model, nsp_dim=300):
        super(ModelNSP_flan, self).__init__()

        # FlanT5 uses a different approach for model identification
        self.model_class = AutoModelForSeq2SeqLM  # Base class for FlanT5 models

        # Load the pretrained FlanT5 model
        self.core_model = AutoModelForSeq2SeqLM.from_pretrained(pretrained_model)
        self.core_model.train()  # Set to training mode
    ...
    def forward(self, input_ids, attention_mask=None, decoder_attention_mask=None, next_sentence_label=None, labels=None):
        # FlanT5 uses decoder_attention_mask instead of token_type_ids
        outputs = self.core_model(
            input_ids=input_ids, attention_mask=attention_mask, decoder_attention_mask=decoder_attention_mask
        )

        # Access the encoder hidden state for NSP task
        encoder_hidden_state = outputs.encoder_hidden_states[0]  # First encoder layer output

        # Average the hidden state across all tokens for NSP
        output = encoder_hidden_state.mean(dim=1)
        logits = self.nsp_head(output)
