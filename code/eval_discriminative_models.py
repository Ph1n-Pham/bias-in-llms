...
def parse_args():
    ...
    pretrained_model_choices = ["google/flan-t5-small",'bert-base-uncased', 'bert-base-cased', "bert-large-uncased-whole-word-masking",
    ...
    tokenizer_choices = ["google/flan-t5-small", "RobertaTokenizer", "BertTokenizer", "XLNetTokenizer"]
...
class BiasEvaluator():
    def __init__(self, pretrained_class="bert-large-uncased-whole-word-masking", no_cuda=False,
                 input_file="../data/dev.json", intrasentence_model="BertLM", #bias.json -> dev.json
        ...
        #tokenizer
        if pretrained_class == "google/flan-t5-small":
            self.TOKENIZER = tokenizer
            self.tokenizer = T5Tokenizer.from_pretrained("google/flan-t5-small", device_map="auto")
            #self.tokenizer.mask_token = "<mask>"
            #self.tokenizer = getattr(transformers, self.TOKENIZER).from_pretrained("bert-large-uncased-whole-word-masking", padding_side="right")
        ...
        if tokenizer == "google/flan-t5-small":
            self.MASK_TOKEN_IDX = self.tokenizer.encode(self.MASK_TOKEN)[0]
        else:
...
if __name__ == "__main__":
    ...
    evaluator = BiasEvaluator(pretrained_class="google/flan-t5-small")#, tokenizer="google/flan-t5-small")
